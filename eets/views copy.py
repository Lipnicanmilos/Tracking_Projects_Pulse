...
def wl_ack(request):
    ...
    sql_ack_None = """
                Select EETS_INTEGRATION_LOG_ITEM_ID, ite.INTEGRATION_LOG_ID, il.EETS_INT_LOG_STATUS_CODE || '-' || il.EETS_INT_LOG_STATUS_NAME as EETS_INT_LOG_STATUS,
                ...
                LEFT JOIN eets_ECM.EXCEPTION_LIST_TYPE_L exl ON ite.EXCEPTION_LIST_TYPE_CODE = exl.EXCEPTION_LIST_TYPE_CODE AND il.language_code = 'SK'
        ...
        if 'generation_pdf' in request.POST:
            ...
            for row in filtered_data_granvia:
                table_data_GRANVIA.append([
                    ...
                    f"{row['EETS_INFRASTRUCTURE']:,.2f}".replace(',', ' '),
                    f"{row['EETS_INFRASTRUCTURE']:,.2f}".replace(',', ' ')
                ])
            table_data_GRANVIA.append([
                'Celková suma:',
                f"{sum_spolu_granvia:,.2f}".replace(',', ' '),
                f"{sum_spolu_granvia:,.2f}".replace(',', ' ')
            ])
            ...
            for row in ppp_data:
                table_data_PPP.append([
                    row['PASSAGEDATE_MONTHLY'],
                    f"{row['SPOLU']:,.2f}".replace(',', ' '),
                    f"{row['SPOLU']:,.2f}".replace(',', ' ')
                ])
            table_data_PPP.append([
                'Celková suma:',
                f"{sum_spolu_ppp:,.2f}".replace(',', ' '),
                f"{sum_spolu_ppp:,.2f}".replace(',', ' ')
            ])
            ...
            for row in filtered_data_stat:
                table_data_stat.append([
                    row['PASSAGEDATE_MONTHLY'],
                    "{:_}".format(row['EETS_INFRASTRUCTURE']).replace('_', ' ')
                    if row['EETS_INFRASTRUCTURE'] is not None else '',
                    "{:_}".format(row['EETS_INFRASTRUCTURE']).replace('_', ' ')
                    if row['EETS_INFRASTRUCTURE'] is not None else ''
                ])
            table_data_stat.append([
                'CELKOVÁ SUMA:',
                "{:_}".format(sum_spolu_stat).replace('_', ' '),
                "{:_}".format(sum_spolu_stat).replace('_', ' ')
            ])
            ...
            for row in spolu_data:
                table_data_spolu.append([
                    row['PASSAGEDATE_MONTHLY'],
                    "{:_}".format(row['SPOLU']).replace('_', ' ') if row['SPOLU'] is not None else '',
                    "{:_}".format(row['SPOLU']).replace('_', ' ') if row['SPOLU'] is not None else ''
                ])
            table_data_spolu.append([
                'CELKOVÁ SUMA:',
                f"{sum_spolu:_}".replace('_', ' '),
                f"{sum_spolu:_}".replace('_', ' ')
            ])
            ...
            # Pridanie riadkov s dátami
            for row in data_co2:
                table_data_CO2.append([
                    row['PASSAGEDATE_MONTHLY'],
                    "{:,.2f}".format(row['EETS_CO2']).replace(',', ' ') if row['EETS_CO2'] is not None else '0.00',
                    "{:,.2f}".format(row['EETS_POLLUTION']).replace(',', ' ') if row['EETS_POLLUTION'] is not None else '0.00',
                    "{:,.2f}".format(row['EETS_INFRASTRUCTURE']).replace(',', ' ') if row['EETS_INFRASTRUCTURE'] is not None else '0.00',
                    "{:,.2f}".format(row['SPOLU']).replace(',', ' ') if row['SPOLU'] is not None else '0.00'
                ])
            ...
            output_filename = f'./Reports/Lego/EETS_LEGO_REP_{MM}2025.pdf'
            ...
            def draw_section_z(c, title, table_data, y_position, colWidths=None):
                ...
                # Tabuľka
                if colWidths is None:
                    colWidths = [1.6*inch, 1.5*inch, 1.5*inch, 1.5*inch, 1.1*inch]
                table = Table(table_data, colWidths=colWidths)
                table.setStyle(table_style)
                table.wrapOn(c, width, height)
                tw, th = table.wrap(0, 0)
                table.drawOn(c, (A4[0] - tw) / 2.6, y_position - th)
                y_position -= (th + 30)
            ...
            # Resetovanie pozície pre novú stranu
            y_position = 750  # Začneme vyššie na novej strane

            # Vykreslenie hlavičky na novej strane (voliteľné, podľa potreby)
            c.drawImage("./eets/static/img/logo_myto.png", 35, 780, width=80, height=50)
            c.setFont("Arial Unicode MS", 8)
            c.drawString(414, 800, "Číslo zostavy: EETS_LEGO_REP_01")
            c.drawString(503, 785, "PDF formát")
            ...
            # c.setFont("Helvetica-Bold", 14)
            # c.drawString(165, 750, "Prerozdelenie mýta medzi vlastníkov ciest - Detailná evidencia EETS")
            ...
            # Vykreslenie detailnej tabuľky na novej strane
            y_position = draw_section_z(c, "Detailná evidencia EETS", table_data_CO2, 690)

            # ... [zvyšok vášho kódu pre footer atď.] ...
            # Červené čiary
            c.setStrokeColorRGB(1, 0, 0)
            c.line(560, 90, 560, 830)
            c.line(35, 90, 560, 90)
            ...
            c.saveState()
            c.setFont("Arial Unicode MS", 7)
            page_info = "Strana (Page) %d/%d" % (c.getPageNumber())
            c.drawRightString(550, 20, page_info)
            c.restoreState()
            ...
            with open(output_filename, 'rb') as f:
                ...
                response['Content-Disposition'] = f'inline; filename="EETS_LEGO_REP_{MM}2025.pdf"'
            return response
